...
SQLite gerenciado pelo Prisma (`prisma/schema.prisma`). O `movieId` que aparece em quase todo modelo é o id da série na TVMaze — junto dele guardamos título, poster e ano para não buscar na TVMaze toda hora.
...
Cada mudança no schema vira uma pasta em `prisma/migrations`. Para aplicar tudo do zero: `npx prisma migrate dev`.
